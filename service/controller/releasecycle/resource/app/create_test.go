...
func Test_newCreateChange(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:          "case 4: nil currentState returns desiredState",
			currentState:  (*applicationv1alpha1.App)(nil),
			desiredState:  fooApp,
		...
		{
			name:          "case 5: empty currentState returns desiredState",
			currentState:  &applicationv1alpha1.App{},
			desiredState:  fooApp,
			expectedAppCR: fooApp,
			errorMatcher:  nil,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			change, err := r.newCreateChange(context.Background(), tc.obj, tc.currentState, tc.desiredState)
